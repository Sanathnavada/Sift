...
def initialize_task_estimate(task: Any) -> None:
    ...
    meta.setdefault("queue_message", "Waiting for a worker slot.")
    ...
    *,
    ...
    for task in task_list:
        ...
        if task.status == "running":
            ...
            task.meta["queue_message"] = f"Running in {lane_label_fn(lane)} lane."
            ...
            if task.meta.get("display_status") == "needs_review":
                ...
                task.meta["queue_message"] = task.status.title()
        _sync_labels(task)
    ...
    for lane, queue in queues_by_lane.items():
        ...
        for index, job in enumerate(queue, start=1):
            ...
            if group_wait > lane_wait:
                task.meta["queue_message"] = "Waiting for the Instagram session lock to become available."
            elif index == 1 and lane_running_count == 0:
                task.meta["queue_message"] = f"Next in {lane_label} lane."
            elif index == 1:
                task.meta["queue_message"] = f"Waiting for the running {lane_label} task to finish."
            else:
                task.meta["queue_message"] = f"Waiting behind {index - 1} task(s) in {lane_label} lane."
