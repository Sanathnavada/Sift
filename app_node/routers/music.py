...
from music_node.models import Track  # noqa: E402
from music_node.services.downloader import MusicDownloader  # noqa: E402
from music_node.services.spotify import SpotifyProvider  # noqa: E402
from music_node.services.youtube import YouTubeResolver  # noqa: E402
...
def _song_job(task_id: str, queries: list[str], outdir: Optional[str]) -> dict:
    ...
    spotify = SpotifyProvider(use_user_auth=False)
...
def _user_library_job() -> dict:
    spotify = SpotifyProvider(use_user_auth=True, allow_interactive=False)
...
def _user_download_job(
    ...
    if selected == ["all"]:
        library = SpotifyProvider(use_user_auth=True, allow_interactive=False).fetch_user_library()
        playlists = library
    else:
        library = None
        playlists = {}
...
def _spotify_callback_html(session, message: str) -> HTMLResponse:
    error_html = f"<p>Error: {session.error}</p>" if session.error else ""
    status_card_url = f"/ui/music/auth/session/{session.id}/card"
    return HTMLResponse(
        content=(
            "<html><body>"
            f"<h2>{message}</h2>"
            f"<p>Status: {session.status}</p>"
            f"{error_html}"
            "<p>Returning to the Gateway Console...</p>"
            "<script>"
            "(function(){"
            ...
            "  } catch (err) {}"
            "}"
            "setTimeout(function(){ window.location.href = '/music'; }, 900);"
            "})();"
            "</script>"
            f"<p><a href='{status_card_url}'>Open authorization status</a></p>"
            "</body></html>"
        )
    )
