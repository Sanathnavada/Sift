...
class YouTubeResolver:
    ...
    def _cached_url(self, cache_key: str) -> Optional[str]:
        with self._cache_lock:
    ...
    def _remember_match(self, cache_key: str, video: VideoResult, score: int) -> None:
        with self._cache_lock:
    ...
    def resolve_track(self, track: Track) -> dict:
        """Resolve one track to a YouTube URL or review candidates."""
        cache_key = self._cache_key(track)
        cached_url = self._cached_url(cache_key)
        if cached_url:
            return {"url": cached_url, "error": None, "candidates": []}
        ...
        if best_video and best_evidence["score"] >= threshold:
            self._remember_match(cache_key, best_video, best_evidence["score"])
            logger.info(
    ...
    def _search_single_with_retry(
        ...
        resolution = self.resolve_track(track)
    ...
    def resolve_all(self, playlists: Dict[str, List[Track]]) -> Dict[str, dict]:
        ...
        for name, tracks in playlists.items():
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                futures = {
                    executor.submit(self.resolve_track, track): track
                    for track in tracks
                }
