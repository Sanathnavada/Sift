...
-----------
  ...
  approve access. The token is then cached in var/music_config/.cache for future runs.
...
-----
  ...
  python -m sift.engines.music.cli --mode song --input "Bohemian Rhapsody Queen"
  python -m sift.engines.music.cli --mode song --input "Blinding Lights The Weeknd"

  # --- MODE: yt ---
  # Download audio by direct YouTube URL (fastest — skips search entirely):
  python -m sift.engines.music.cli --mode yt --input "https://youtu.be/_yhBlE3D9cc" --outdir D:\Music\bahava_bhaktigeete\venkatesha_stotram

  # Download audio by video name (runs keyword search, same engine as 'song' mode):
  python -m sift.engines.music.cli --mode yt --input "Rick Astley Never Gonna Give You Up" --outdir ./downloads

  # Ephemeral — no --outdir → file saved to system temp, path printed (for future UI use):
  python -m sift.engines.music.cli --mode yt --input "https://youtu.be/dQw4w9WgXcQ"
...
OUTPUT
------
  Music files are saved to the directory configured in sift.engines.music.config (or --outdir for yt mode).
...
from sift.engines.music.config import get_logger, SPOTIFY_BACKUP_FILE
