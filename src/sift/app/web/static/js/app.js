(function () {
  ...
  var spotifyAuthPopup = null;
  var spotifyAuthPopupWatch = null;
  var spotifyAuthCloseUrl = "";
  var spotifyAuthPollUrl = "";
  ...
  function toSameOriginAbsoluteUrl(url) {
    if (!url) {
      return url;
    }
    try {
  ...
  function openInstagramAuthPopup(url, closeUrl) {
    ...
    var popupUrl = toSameOriginAbsoluteUrl(url);
  ...
  function navigateInstagramAuthPopup(url, closeUrl) {
    ...
    instagramAuthClosingExpected = false;
    instagramAuthPopup.location.href = withCacheBuster(toSameOriginAbsoluteUrl(url));
    try {
      instagramAuthPopup.focus();
    } catch (error) {
  ...
  function stopSpotifyAuthPopupWatch() {
    if (spotifyAuthPopupWatch) {
      window.clearInterval(spotifyAuthPopupWatch);
      spotifyAuthPopupWatch = null;
    }
  }

  function refreshSpotifyAuthPanel(statusUrl) {
    if (statusUrl && updateSpotifyAuthStatus(statusUrl)) {
      return;
    }
    consumeSpotifyAuthStatus();
  }

  function notifySpotifyAuthPopupClosed() {
    if (!spotifyAuthCloseUrl) {
      refreshSpotifyAuthPanel(spotifyAuthPollUrl);
      return;
    }
    var target = document.querySelector("#music-library-auth-panel");
    var headers = { "X-Client-Session-ID": getClientSessionId() };
    try {
      fetch(spotifyAuthCloseUrl, {
        method: "POST",
        headers: headers,
        credentials: "same-origin"
  ...
  function watchSpotifyAuthPopupClose() {
    stopSpotifyAuthPopupWatch();
    if (!spotifyAuthPopup) {
      return;
    }
    spotifyAuthPopupWatch = window.setInterval(function () {
      if (!spotifyAuthPopup || spotifyAuthPopup.closed) {
        stopSpotifyAuthPopupWatch();
        notifySpotifyAuthPopupClosed();
      }
    }, 1000);
  }

  function openSpotifyAuthPopup(link) {
    spotifyAuthCloseUrl = link.dataset.spotifyAuthCloseUrl || "";
    spotifyAuthPollUrl = link.dataset.spotifyAuthPollUrl || "";

    // If the OAuth callback could not close an older popup, that popup may now
    // contain the full /music page.  When the user clicks from that window, do
    // not open another nested app window; navigate the current popup to Spotify.
    if (window.opener && window.name && window.name.indexOf("spotify-auth-") === 0) {
      window.location.assign(link.href);
      return;
    }

    // Use a fresh popup name per attempt. A fixed name can reuse a stale popup
    // that is already sitting on /music, which makes it look like the approval
    // action opened Sift instead of Spotify.
    var popupName = "spotify-auth-" + Date.now();
    spotifyAuthPopup = window.open(toSameOriginAbsoluteUrl(link.href), popupName, "popup=yes,width=520,height=760");
    if (spotifyAuthPopup) {
      try {
        spotifyAuthPopup.focus();
      } catch (error) {
        // Browser may restrict popup focus; the close watcher still works.
      }
      watchSpotifyAuthPopupClose();
    } else {
      refreshSpotifyAuthPanel(spotifyAuthPollUrl);
    }
  }
